...
<body class="is-preload">
   ...
   <article id="main">
     ...
     <form name="reFindPwForm"
		action="${pageContext.request.contextPath}/user/UserChangePwOk.us"
		method="post">
...
<br>
    ...
    <script>
				$("a.sendPw").on("click", function(e) {
					...
					if (reFindPwForm.user_id.value == "") {
						alert("아이디를 입력해주세요.");
						return false;
					}
					if (reFindPwForm.temp_pw.value == "") {
						alert("전송받으신 임시번호를 입력해주세요.");
						return false;
					}

					if (reFindPwForm.new_pw.value == "") {
						alert("앞으로 사용하실 비밀번호를 입력해 주세요.");
						return false;
					}else{var reg = /^(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$/;
					if(!reg.test(reFindPwForm.new_pw.value)){
						alert("비밀번호는 8자리 이상이어야 하며, 소문자/숫자/특수문자 모두 포함해야 합니다.");
						reFindPwForm.new_pw.focus();
						return false; }}
